...
app.get("/", function(req, res) {
  ...
  Item.find({}, function(err, foundItems){  //in this case we get the result which is array 
    ...
    if (foundItems.length === 0) {
      ...
      res.redirect("/");  //bcz hm if statement k baad else p aayenge to founditems(bcz founditem upr specify kra h mene) m items nhi honge 
                          //  that's why hmne redirect kra home route ko taki founditems m item store ho jaye!    
    } else {
...
app.get("/:customListName", function(req, res){
  ...
  List.findOne({name: customListName}, function(err, foundList){  //In this case we get the result which is object
    if (!err){                                                    //bcz we find only one
      if (!foundList){
        //Create a new list
        const list = new List({
          name: customListName,
